...
st.title("Entry Data Produksi Hutan Rakyat")
...
client.session = gspread.auth.ClientSession(creds)

# Open the Google Sheet
spreadsheet = client.open_by_url("https://docs.google.com/spreadsheets/d/1H-VZu6FYp8guymrb6pKJBw-D5rLpdvbViczf-s-1SUM")
...
kabupaten_df = pd.DataFrame(Kabupaten_list, columns=['kabupaten_id', 'provinsi_id', 'kabupaten_name'])
...
# Form untuk input
with st.form(key="Form_Dataproduksihutanrakyat"):
    Kabupaten = st.selectbox(
        label="Kabupaten/Kota",
        options=[""] + kabupaten_df["kabupaten_name"].tolist(),
        key="kabupaten_name",
    )
    ...
    # Input lainnya
    Desa = st.text_input(label="Desa")
    kth = st.text_input(label="KTH")
    Penyuluh = st.text_input(label="Penyuluh pendamping")
    Laporan_prodkayubulat = st.number_input(label="Produksi Kayu Bulat (m3)", min_value=0.0)
    data_NTE = st.number_input(label="Data NTE", min_value=0.0)
    selisih = data_NTE - Laporan_prodkayubulat
    Keterangan = st.text_input(label="Keterangan lainnya")
    ...
    if st.form_submit_button(label="Konfirmasi laporan"):
        if not Kabupaten or not Kecamatan or not Desa or not kth or not Penyuluh:
                ...
                [
                    {
                        ...
                        "Laporan Produksi Kayu Bulat (m3)": Laporan_prodkayubulat,
            ...
            worksheet.append_rows(dataprodhutanrakyat.values.tolist(), value_input_option="USER_ENTERED")
